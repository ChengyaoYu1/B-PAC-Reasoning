...
while getopts ":d:m:p:s:r:t:o:e:f:b:x:g:" opt; do
  case $opt in
    d) DATA_NAME="$OPTARG"
    ;;
    m) model_name="$OPTARG"
...
if [ -z "$DATA_NAME" ] || [ -z "$model_name" ] || [ -z "$model_pretty_name" ] || [ -z "$n_shards" ]; then
  echo "Usage: $0 -d DATA_NAME -m model_name -p model_pretty_name [-s n_shards] [-g gpu_ids] [-r run_name] [-t TEMP] [-o TOP_P] [-e rp] [-b batch_size]"
  exit 1
fi
...
if [ "$run_name" = "default" ]; then
    output_dir="result_dirs_rebutal/${DATA_NAME}/" 
else
    output_dir="result_dirs_rebutal/${DATA_NAME}/${run_name}/" 
fi
...
if [ $n_shards -eq 1 ]; then
    ...
    if [ "$n_shards" -gt "$gpu_count" ]; then
        echo "n_shards ($n_shards) is larger than available gpu ids ($gpu_count): $GPU_IDS"
        exit 1
    fi
    num_gpus=1 

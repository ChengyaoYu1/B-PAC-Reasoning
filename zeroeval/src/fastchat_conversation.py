...
def map_to_conv(model_name):
    ...
    if model_name in HF_TEMPLATED_MODELS:
        try:
            ...
            # Some newer/odd tokenizer_class configs (e.g. "TokenizersBackend") might not be
            # compatible with the currently installed transformers/vllm stack.
            # In that case, fall back to the local prompt templates below.
            print(
                f"Warning: HF chat-template failed for {model_name}: {type(e).__name__}: {e}. "

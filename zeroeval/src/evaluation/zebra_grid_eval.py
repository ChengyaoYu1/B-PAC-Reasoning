...
def load_private_solutions():
    ...
    private_path = Path(os.environ.get("ZEROEVAL_ZEBRA_PRIVATE_PATH", "data/zebra_grid_private.parquet"))
    if not private_path.is_file():
        raise FileNotFoundError(
            f"Missing private ZebraLogic parquet file: {private_path}. "
            "Set ZEROEVAL_ZEBRA_PRIVATE_PATH to the correct path."
        )
    private_zebra_data = load_dataset("parquet", data_files=str(private_path), split="train")

...
def analyze_logprobs(seq):
    ...
    for gen_index in range(len(seq_logprobs)):
        lp_dict = seq_logprobs[gen_index]
        vals = [v.logprob for v in lp_dict.values()]
        vals_tensor = torch.tensor(vals, dtype=torch.float32)
        probs = torch.softmax(vals_tensor, dim=-1)
        max_probs.append(probs.max())
        topk_entropies.append(
            -(probs * torch.log(probs.clamp_min(1e-12))).sum()
        )

        chosen_id = token_ids[gen_index]
        chosen_logprob_item = lp_dict.get(chosen_id)
        if chosen_logprob_item is None:
            continue
        logprobs.append(chosen_logprob_item.logprob)

    if len(logprobs) == 0:
        return None, None, None, None

    avg_max_prob = torch.stack(max_probs).mean().item()
    logprobs_tensor = torch.tensor(logprobs, dtype=torch.float32)
    probs_tensor = torch.exp(logprobs_tensor)
    avg_prob = probs_tensor.mean().item()
    avg_logprob = logprobs_tensor.mean().item()
    avg_topk_entropy = torch.stack(topk_entropies).mean().item()
    return avg_max_prob, avg_prob, avg_logprob, avg_topk_entropy
...
if __name__ == "__main__":
    ...
    hf_tokenizer = None
    need_hf_tokenizer = (args.model_name in IM_END_MODELS) or (args.model_name in HF_TEMPLATED_MODELS)
    if need_hf_tokenizer:
        try:
            hf_tokenizer = AutoTokenizer.from_pretrained(args.model_name, trust_remote_code=True)
        except Exception as e:
            print(
                f"Warning: failed to load HF tokenizer for stop-token setup: "
                f"{type(e).__name__}: {e}. Continue without tokenizer-derived stop ids."
            )

    if args.model_name in IM_END_MODELS and hf_tokenizer is not None:
    ...
    if args.model_name in HF_TEMPLATED_MODELS and hf_tokenizer is not None:
    ...
    if os.path.exists(filepath) and not args.overwrite:
        ...
        for output_item in formatted_outputs:
            ...
            token_logprobs.append(output_item.get("token_probs"))
            topk_logprobs.append(output_item.get("topk_probs"))
            avg_logprobs.append(output_item.get("avg_logprob"))
            avg_topk_entropies.append(output_item.get("avg_topk_entropy"))
    ...
    if args.engine == "vllm":
        ...
        sampling_params = SamplingParams(top_p=args.top_p, temperature=args.temperature,            repetition_penalty=args.repetition_penalty, max_tokens=args.max_tokens,
                                         ...
                                         top_k=20,
                                         logprobs=20)
        # 准备元数据容器：生成 token 数与耗时（毫秒）
        if "gen_token_count" not in metadata:
            metadata["gen_token_count"] = [None] * num_skipped
        if "latency_s" not in metadata:
            metadata["latency_s"] = [None] * num_skipped
        # # 存储所有输出token的logprobs
        # if "full_token_logprobs" not in metadata:
        #     metadata["full_token_logprobs"] = [None] * num_skipped
        ...
        for cur_id in tqdm(range(0, len(todo_inputs), args.batch_size), desc=f"Generating {args.model_name} from {args.start_index} to {args.end_index}"):
            ...
            for output in batch_outputs:
                ...
                the_topk_logprobs = []
                the_avg_logprobs = []
                the_avg_topk_entropies = []
                # current_full_logprobs = []

                # 统计该样本生成 token 数（若 n>1，则为每个候选的 token 数列表）
                gen_token_counts = []
                for seq in output.outputs:
                    avg_max_prob, avg_chosen_prob, avg_chosen_logprob, avg_topk_entropy = analyze_logprobs(seq)
                    the_token_logprobs.append(avg_chosen_prob)
                    the_topk_logprobs.append(avg_max_prob)
                    the_avg_logprobs.append(avg_chosen_logprob)
                    the_avg_topk_entropies.append(avg_topk_entropy)
                    gen_token_counts.append(len(seq.token_ids))
                    
                token_logprobs.append(the_token_logprobs)
                topk_logprobs.append(the_topk_logprobs)
                avg_logprobs.append(the_avg_logprobs)
                avg_topk_entropies.append(the_avg_topk_entropies)
                metadata["gen_token_count"].append(gen_token_counts if len(gen_token_counts) > 1 else gen_token_counts[0])
                metadata["latency_s"].append(elapsed_s)

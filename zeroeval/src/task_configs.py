...
from _TEMPLATES import apply_mc_template, apply_lgp_grid_template, apply_oeqa_template, apply_gplanet_template,apply_magpie_template


def local_parquet(env_name, relative_path):
    path = Path(os.environ.get(env_name, relative_path))
    if not path.is_file():
        raise FileNotFoundError(
            f"Missing dataset file: {path}. Set {env_name} to the correct parquet path."
        )
    return str(path)
...
def mapping_task_names(data_name):
    ...
    elif data_name == "mmlupro":
        mmlupro_data_path = local_parquet("ZEROEVAL_MMLUPRO_PATH", "data/mmlupro_reconstructed.parquet")
        dataset = load_dataset("parquet", data_files=mmlupro_data_path, split="train")
    elif data_name == "math":
        dataset = load_dataset("parquet", data_files=local_parquet("ZEROEVAL_MATH_PATH", "data/math.parquet"), split="train")
    elif data_name == "zebra-grid":
        dataset = load_dataset("parquet", data_files=local_parquet("ZEROEVAL_ZEBRA_GRID_PATH", "data/zebra_grid.parquet"), split="train")
    elif data_name == "bbh":
        dataset = load_dataset("parquet", data_files=local_parquet("ZEROEVAL_BBH_PATH", "data/bbh.parquet"), split="train")
    elif data_name =="magpie":
        magpie_data_path = local_parquet("ZEROEVAL_MAGPIE_PATH", "data/magpie_reconstructed.parquet")
        dataset = load_dataset("parquet", data_files=magpie_data_path, split="train")
...
def prompt_generation(data_name, data_item, args):
    ...
    """
    if data_name in ["mmlu-redux","mmlupro"]:  # and other multiple-choice QA dataset 
    ...
    elif data_name in ["gsm", "math-l5","math","bbh"]:
        question_key = "question"
        if data_name in ["math-l5", "math"]:
            question_key = "problem"
        prompt = apply_oeqa_template(data_item, question_key = question_key)
    elif data_name in ["magpie"]:
        prompt = apply_magpie_template(data_item)
